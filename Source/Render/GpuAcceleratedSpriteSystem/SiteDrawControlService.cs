                    ...
                    for (int y = 0; y < site.Size.Y; y++)
                    {
                        ...
                        if (tile != Entity.Null && tile.TryGet(out bcc))
                        {
                            ...
                            Material mat = bcc.Material;
                            {
                                ...
                                _siteRenderer.StaticDrawer.AddTileSprite(LAYER, tilePos, sprite, col);

                                Entity tmp;
                                // Draw borders of Wall
                                if (site.Map.TryGet(tilePos - new Point3(1, 0, 0), out tmp) && tmp != Entity.Null &&
                                        !tmp.Has<BaseConstruction>())
                                    _siteRenderer.StaticDrawer.AddTileSprite(LAYER, tilePos, lborderSprite, borderColor);
                                if (site.Map.TryGet(tilePos - new Point3(0, 1, 0), out tmp) && tmp != Entity.Null &&
                                        !tmp.Has<BaseConstruction>())
                                    _siteRenderer.StaticDrawer.AddTileSprite(LAYER, tilePos, rborderSprite, borderColor, new Vector3(GlobalResources.Settings.TileSize.X / 2, 0, 0));
                            }
                            {
                                ...
                                Sprite sprite = GlobalResources.GetResourceBy(GlobalResources.Sprites, "ID",
                                        constr.Sprites[spart][rand % constr.Sprites[spart].Count]);
                                Color col = constr.HasMaterialColor ? mat.Color : Color.White;
                                _siteRenderer.StaticDrawer.AddTileSprite(LAYER, tilePos, sprite, col, new Vector3(0, -GlobalResources.Settings.FloorYoffset, 0));
                                ...
                                if (site.Map.TryGet(tilePos - new Point3(0, 1, 0), out tmp) && tmp != Entity.Null &&
                                        !tmp.Has<BaseConstruction>())
                                    _siteRenderer.StaticDrawer.AddTileSprite(LAYER, tilePos, rborderSprite, borderColor,
                                ...
                                if (tile.TryGet(out hveg))
                                {
                                    ...
                                    if (spritesIDs != null)
                                        _siteRenderer.StaticDrawer.AddTileSprite(LAYER, tilePos, sprite, Color.White,
